...
// 医生发起患者转诊状态
const (
	ZhuanzhenInfo_State_NewPublish = "NewPublish"
	ZhuanzhenInfo_State_EndrWaitReceive = "EndrWaitSign"
	ZhuanzhenInfo_State_EndrReceived = "EndrSigned"
	ZhuanzhenInfo_State_EndrReject = "EndrReject"
	ZzhuanzhenInfo_State_EndrTimeout = "EndrTimeout"
)
// 患者信息key的前缀
const Patient_Prefix = "Patientkey_" 

// search表的映射名
const IndexName = "holderName~patientNo"
...
func (a *BillChaincode) putBill(stub shim.ChaincodeStubInterface, bill Bill) ([]byte, bool) {
	...
	err = stub.PutState(Patient_Prefix + bill.BillInfoID, byte)
	if err!=nil {
		return nil, false
	}
	return byte, true
...
func (a *BillChaincode) issue(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	bill.State = ZhuanzhenInfo_State_NewPublish
    // 保存票据
...
func (a *BillChaincode) endorse(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	resultsIterator, err := stub.GetHistoryForKey(Patient_Prefix+args[0])
	if err != nil {
	...
	bill.State = ZhuanzhenInfo_State_EndrWaitReceive
    // 保存票据
	_, bl = a.putBill(stub, bill)
	if !bl {
...
func (a *BillChaincode) accept(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	bill.State = ZhuanzhenInfo_State_EndrReceived
    // 保存票据
	_, bl = a.putBill(stub, bill)
	if !bl {
...
func (a *BillChaincode) reject(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	bill.State = ZhuanzhenInfo_State_EndrReject
    // 保存票据
	_, bl = a.putBill(stub, bill)
	if !bl {
...
func (a *BillChaincode) queryMyWaitBill(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for billsIterator.HasNext() {
		...
		if bill.State == ZhuanzhenInfo_State_EndrWaitReceive && bill.WaitEndorserCmID == args[0] {
			billList = append(billList, bill)
		}
	}
    // 取得并返回票据数组 
